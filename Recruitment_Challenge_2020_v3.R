...
visualise <- function(start_date,finish_date=1){
  ...
  times <- data.table(colnames(QLD_hist_demand))
  QLD_hist_avg <- data.table(colMeans(QLD_hist_demand), keep.rownames = TRUE)
  colnames(QLD_hist_avg) <- "Avg"
  QLD_hist_avg$Time <- times
  ...
  if(finish_date==1){
    ...
    colnames(data_sel_avg)<-"Avg"
      ...
      data_sel_avg <- data_sel       %>%
        ...
        data.table()
      colnames(data_sel_avg)<-"Avg"
      data_sel_avg$Time <- times
      
      }
  
  # Plot Averages
  x_values<-QLD_hist_avg$Time
  ggplot() + geom_point(data=QLD_hist_avg, aes(x=Time,y=Avg, color = "red")) +
    geom_point(data = data_sel_avg, aes(x=Time,y=Avg, na.rm=TRUE,color="blue")) +
